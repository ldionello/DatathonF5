...
import pandas as pd
from modelo import treinar_modelo, prever_candidato

palavras_chave = ['python', 'java', 'javascript', 'c++', 'c#', 'php', 'ruby', 'go', 'rust', 'kotlin',
...
nivel_ingles = st.selectbox('Nível de Inglês', [
    'Nenhum', 'Básico', 'Intermediário', 'Avançado', 'Fluente'])
nivel_espanhol = st.selectbox('Nível de Espanhol', [
    'Nenhum', 'Básico', 'Intermediário', 'Avançado', 'Fluente'])
area_atuacao = st.multiselect('Área de Atuação', [
    'TI - Desenvolvimento/Programação-', 'TI - SAP-', 'TI - Projetos-',
    'TI - Infraestrutura-', 'Gestão e Alocação de Recursos de TI-', 'Outro'])
tipo_contratacao = st.selectbox('Tipo de Contratação', [
    'CLT Full', 'PJ/Autônomo', 'Outro'])
nivel_profissional = st.selectbox('Nível Profissional', [
    'Pleno', 'Sênior', 'Júnior'])

keywords_encontradas = st.multiselect(
    'Skills Candidato', sorted(palavras_chave))
keywords_vaga = st.multiselect('Skills da Vaga', sorted(palavras_chave))
...
if st.button('Prever'):
    ...
    pred, proba = prever_candidato(modelo, preprocessor, input_data)
